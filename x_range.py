...
gamma = 1
beta = 1  # 假设 beta 为常数
num_simulations = 500  # 运行的模拟次数
num_x_steps = 1000  # 固定的 x 步数
T = 10
end_t = 0
sample_number = 20
fixed_dt = 0.1  # 固定时间步长
...
errors_D2_limits = []
errors_D3_limits = []
errors_D1_transformed_limits = []
errors_D2_transformed_limits = []
errors_D3_transformed_limits = []

for (x_start, x_end) in x_limits:
    ...
    errors_D1_limits.append(average_cdf_diff_D1)
    ...
    errors_D1_transformed_limits.append(average_cdf_diff_D1_transformed)
    ...
    errors_D2_limits.append(average_cdf_diff_D2)
    ...
    errors_D2_transformed_limits.append(average_cdf_diff_D2_transformed)
    ...
    errors_D3_limits.append(average_cdf_diff_D3)
    ...
    errors_D3_transformed_limits.append(average_cdf_diff_D3_transformed)
    
    end_time = time.time()
    print(f"x_start = {x_start}, x_end = {x_end}, Iteration Time = {end_time - start_time:.4f} seconds")

# 绘制误差与 (x_start, x_end) 范围的关系图
x_labels = [f"{x_start},{x_end}" for (x_start, x_end) in x_limits]

plt.figure(figsize=(10, 6))
plt.plot(x_labels, errors_D1_limits, marker='o', color=colors['D1'], label='Error UBU')
plt.plot(x_labels, errors_D1_transformed_limits, marker='o', color=colors['D1_transformed'], label='Error UBU_transformed')
plt.plot(x_labels, errors_D2_limits, marker='o', color=colors['D2'], label='Error BAOAB')
plt.plot(x_labels, errors_D2_transformed_limits, marker='o', color=colors['D2_transformed'], label='Error BAOAB_transformed')
plt.plot(x_labels, errors_D3_limits, marker='o', color=colors['D3'], label='Error BUB')
plt.plot(x_labels, errors_D3_transformed_limits, marker='o', color=colors['D3_transformed'], label='Error BUB_transformed')
plt.xlabel('(x_start, x_end)')
plt.ylabel('Average CDF Difference (Error)')
plt.title('Error vs. x Range (x_start, x_end)')
plt.legend()
