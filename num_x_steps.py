...
# 定义不同的 num_x_steps
num_x_steps_values = [ 400, 800, 1600 , 2400, 3200, 4800]
fixed_dt = 0.1  # 假设你要研究的时间步长为 0.1
...
errors_D2_steps = []
errors_D3_steps = []
errors_D1_transformed_steps = []
errors_D2_transformed_steps = []
errors_D3_transformed_steps = []
...
# 选择不同的 num_x_steps 值
num_x_steps_values = [500, 1000, 2000, 4000, 8000]
...
for num_x_steps in num_x_steps_values:
    x_values = np.linspace(x_start, x_end, num_x_steps)
    x_real = compute_x_real(x_values)
    ...
    for _ in range(num_simulations):
        ...
        # 运行模拟并保留最后20个数据点
        final_x_values_D1.append(run_simulation_D1(fixed_dt, end_t, gamma, beta, second_iteration, D1, initial)[-sample_number:])
        final_x_values_D1_transformed.append(run_simulation_D1_transformed(fixed_dt, end_t, gamma, beta, second_iteration_transformed, D1, dynamic_time_step, initial)[-sample_number:])
        final_x_values_D2.append(run_simulation_D2(fixed_dt, end_t, gamma, beta, second_iteration, D2, initial)[-sample_number:])
        final_x_values_D2_transformed.append(run_simulation_D2_transformed(fixed_dt, end_t, gamma, beta, second_iteration_transformed, D2, dynamic_time_step, initial)[-sample_number:])
        final_x_values_D3.append(run_simulation_D3(fixed_dt, end_t, gamma, beta, second_iteration, D3, initial)[-sample_number:])
        final_x_values_D3_transformed.append(run_simulation_D3_transformed(fixed_dt, end_t, gamma, beta, second_iteration_transformed, D3, dynamic_time_step, initial)[-sample_number:])
    ...
    errors_D1_steps.append(average_cdf_diff_D1)
    ...
    errors_D1_transformed_steps.append(average_cdf_diff_D1_transformed)
    ...
    errors_D2_steps.append(average_cdf_diff_D2)
    ...
    errors_D2_transformed_steps.append(average_cdf_diff_D2_transformed)
    ...
    errors_D3_steps.append(average_cdf_diff_D3)
    ...
    errors_D3_transformed_steps.append(average_cdf_diff_D3_transformed)
    
    end_time = time.time()
    print(f"num_x_steps = {num_x_steps}, Iteration Time = {end_time - start_time:.4f} seconds")

# 绘制误差与 num_x_steps 的关系图
plt.figure()
plt.plot(num_x_steps_values, errors_D1_steps, marker='o', color=colors['D1'], label='Error UBU')
plt.plot(num_x_steps_values, errors_D1_transformed_steps, marker='o', color=colors['D1_transformed'], label='Error UBU_transformed')
plt.plot(num_x_steps_values, errors_D2_steps, marker='o', color=colors['D2'], label='Error BAOAB')
plt.plot(num_x_steps_values, errors_D2_transformed_steps, marker='o', color=colors['D2_transformed'], label='Error BAOAB_transformed')
plt.plot(num_x_steps_values, errors_D3_steps, marker='o', color=colors['D3'], label='Error BUB')
plt.plot(num_x_steps_values, errors_D3_transformed_steps, marker='o', color=colors['D3_transformed'], label='Error BUB_transformed')
plt.xlabel('Number of x Steps (num_x_steps)')
plt.ylabel('Average CDF Difference (Error)')
plt.title('Error vs. Number of x Steps (num_x_steps)')
plt.legend()
plt.show()
