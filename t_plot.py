...
# 运行模拟并保留所有迭代数据
initial = np.random.normal(1000, 1)
...
x_values_D1_transformed,cumulative_t_D1_transformed  = run_simulation_D1_transformed(fixed_dt, end_t, gamma, beta, second_iteration_transformed, D1,  initial,k_max,k_function)
x_values_D2_transformed,cumulative_t_D2_transformed = run_simulation_D2_transformed(fixed_dt, end_t, gamma, beta, second_iteration_transformed, D2,  initial,k_max,k_function)
x_values_D3_transformed,cumulative_t_D3_transformed = run_simulation_D3_transformed(fixed_dt, end_t, gamma, beta, second_iteration_transformed, D3,  initial,k_max,k_function)
# 计算每次迭代的 delta_t，并进行累加
# 计算前三种方法的累积时间 (固定步长)
cumulative_t_D1 = np.cumsum([fixed_dt] * len(x_values_D1))
cumulative_t_D2 = np.cumsum([fixed_dt] * len(x_values_D2))
cumulative_t_D3 = np.cumsum([fixed_dt] * len(x_values_D3))

# 绘制图形
plt.figure(figsize=(12, 8))
plt.plot(cumulative_t_D1, x_values_D1, label='x values D1')
plt.plot(cumulative_t_D2, x_values_D2, label='x values D2')
plt.plot(cumulative_t_D3, x_values_D3, label='x values D3')
plt.plot(cumulative_t_D1_transformed, x_values_D1_transformed, label='x values D1 Transformed', linestyle='--')
plt.plot(cumulative_t_D2_transformed, x_values_D2_transformed, label='x values D2 Transformed', linestyle='--')
plt.plot(cumulative_t_D3_transformed, x_values_D3_transformed, label='x values D3 Transformed', linestyle='--')

plt.xlabel('Cumulative Time (t)')
plt.ylabel('x values')
plt.title('x values over Cumulative Time')
